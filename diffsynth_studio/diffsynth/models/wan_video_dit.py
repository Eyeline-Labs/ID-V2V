...
from .prope import prope_attention
...
class ReferenceCrossAttention(nn.Module):
    def __init__(self, dim: int, inner_dim: int, num_heads: int, eps: float = 1e-6):
        super().__init__()
        self.dim = dim
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, cam_emb=None, context2=None):
        ...
        # Encode camera
        encode_camera = cam_emb is not None and hasattr(self, "camera_encoding")
        if encode_camera and hasattr(self, "cam_encoder") and self.camera_encoding in ("recammaster", "plucker"):
            cam_emb = self.cam_encoder(cam_emb)
            input_x = input_x + cam_emb

        x_self_attn = self.self_attn(input_x, freqs)

        if encode_camera and hasattr(self, "cam_encoder") and self.camera_encoding in ("prope_nograd", "prope_grad"):
            with {"prope_nograd": torch.no_grad(), "prope_grad": nullcontext()}[self.camera_encoding]:
                x_prop_self_attn = prope_attention(self.self_attn, input_x, cam_emb)
            x_self_attn = x_self_attn + self.cam_encoder(x_prop_self_attn)

        # Project self-attention with camera embedding output
        if encode_camera and hasattr(self, "projector"):
        ...
        # Inject reference video (if not through self-attention)
        if context2 is not None and hasattr(self, "cross_attn2") and hasattr(self, "norm4"):
            x = x + self.cross_attn2(self.norm4(x), context2)
...
class WanModel(torch.nn.Module):
    ...
    def set_ntk(self, ntk_alpha: list[float] = [1.0, 5.0, 5.0]):
        # TODO: Usually the last two values of ntk_alpha are the same, possible options are 5.0, 10.0, 20.0
        # For V2V, 5.0 seems to be the best, 20.0 is better for T2V
        # Start with 5.0, but if there is room try 10.0 and 20.0 too
