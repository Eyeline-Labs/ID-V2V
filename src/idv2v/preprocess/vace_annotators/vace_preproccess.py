...
# <<<<< A standalone function to call outside of this file <<<<<
def run_vace_preprocess(
    task,
    video=None,
    image=None,
    mask=None,
    ...
    """
    Run VACE preprocessing as a standalone function that can be imported and used in other Python files.
    
    Args:
        task (str): The task to run (must be in VACE_PREPROCCESS_CONFIGS)
        video (str, optional): The path of the videos to be processed, separated by commas if there are multiple
        image (str, optional): The path of the images to be processed, separated by commas if there are multiple
        mask (str, optional): The path of the mask images to be processed, separated by commas if there are multiple
        bbox (list, optional): List of bounding boxes, each as [x1, y1, x2, y2]
        caption (str, optional): The caption to be processed
        label (str, optional): The label to be processed, separated by commas if there are multiple
        mode (str, optional): The specific mode of the task
        direction (str, optional): The direction of outpainting, separated by commas
        expand_ratio (float, optional): The outpainting's outward expansion ratio
        expand_num (int, optional): The number of frames extended by the extension task
        maskaug_mode (str, optional): The mode of mask augmentation
        maskaug_ratio (float, optional): The ratio of mask augmentation
        pre_save_dir (str, optional): The path to save the processed data
        save_fps (int, optional): The fps to save the processed data (default: 16)
    
    Returns:
        dict: Dictionary containing the paths of saved results
    """
    # Create a namespace object with the provided parameters
    class Args:
    ...
    args = Args(
        ...
        maskaug_mode=maskaug_mode,
        maskaug_ratio=maskaug_ratio,
