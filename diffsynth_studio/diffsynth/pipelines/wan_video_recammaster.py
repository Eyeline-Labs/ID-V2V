...
class BasePipeline(torch.nn.Module):
    ...
    def freeze_except(self, module_keywords):
...
class WanVideoReCamMasterPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_ReCamMasterReference(),
    ...
    def enable_usp(self, use_cinescale=False):
        from xfuser.core.distributed import get_sequence_parallel_world_size
        from ..distributed.xdit_context_parallel import usp_attn_forward, usp_dit_forward

        usp_attn_forward_fn = partial(usp_attn_forward, use_cinescale=use_cinescale)
        for block in self.dit.blocks:
            block.self_attn.forward = types.MethodType(usp_attn_forward_fn, block.self_attn)
    ...
    def from_pretrained(
        ...
        camera_encoding: str = "recammaster",
        use_usp=False,
    ):
        # Redirect model path
        ...
        assert camera_encoding in ("recammaster", "plucker", "prope_nograd", "prope_grad")
        pipe.camera_encoding = camera_encoding
        ...
        # Add ReCamMaster modules
        dim = pipe.dit.blocks[0].self_attn.q.weight.shape[0]
        for block in pipe.dit.blocks:
            block.camera_encoding = camera_encoding
            # Camera encoder
            if camera_encoding == "recammaster":
                block.cam_encoder = torch.nn.Linear(12, dim)
            elif camera_encoding == "plucker":
                block.cam_encoder = torch.nn.Linear(6, dim)
            elif camera_encoding in ("prope_nograd", "prope_grad"):
                block.cam_encoder = torch.nn.Linear(dim, dim)  # TODO: Maybe gating since this is expensive?
            block.cam_encoder.weight.data.zero_()
            block.cam_encoder.bias.data.zero_()
            # Self-attention projector (after adding camera embeddings)
        ...
        # Load custom checkpoint (can include ReCamMaster params), TODO: Set strict=False?
    ...
    def __call__(
        ...
        # ReCamMaster
        reference_video: Optional[np.ndarray] = None,
        cam_c2w: Optional[np.ndarray] = None,  # f 4 4
        intrinsics: Optional[np.ndarray] = None,  # f 4
        # ControlNet
        ...
        inputs_shared = {
            ...
            "reference_video": reference_video, "cam_c2w": cam_c2w, "intrinsics": intrinsics,
            "camera_control_direction": camera_control_direction, "camera_control_speed": camera_control_speed, "camera_control_origin": camera_control_origin,
            "vace_video": vace_video, "vace_video_mask": vace_video_mask, "vace_reference_image": vace_reference_image, "vace_scale": vace_scale,
            "seed": seed, "rand_device": rand_device,
            "height": height, "width": width, "num_frames": num_frames, "frame_interval": frame_interval,
...
class PipelineUnit:
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, inputs: dict, positive=True, **kwargs) -> dict:
...
class PipelineUnitRunner:
    ...
    def __call__(self, unit: PipelineUnit, pipe: WanVideoReCamMasterPipeline, inputs_shared: dict, inputs_posi: dict, inputs_nega: dict) -> tuple[dict, dict]:
        if unit.take_over:
...
class WanVideoUnit_ShapeChecker(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, height, width, num_frames):
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, prompt, positive) -> dict:
        if prompt is None:
            return {}
...
class WanVideoUnit_ImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
        if input_image is None:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        image = pipe.preprocess_image(crop_and_resize([input_image], height, width)[0]).to(pipe.device)
        clip_context = pipe.image_encoder.encode_image([image])
        msk = torch.ones(1, num_frames, height//8, width//8, device=pipe.device)
        msk[:, 1:] = 0
        if end_image is not None:
            end_image = pipe.preprocess_image(crop_and_resize([end_image], height, width)[0]).to(pipe.device)
...
class WanVideoUnit_FunControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, control_video, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y):
...
class WanVideoUnit_FunReference(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, reference_image, height, width):
        if reference_image is None:
            return {}
        pipe.load_models_to_device(["vae"])
        reference_image = crop_and_resize([reference_image], height, width)[0]
...
class WanVideoUnit_FunCameraControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, height, width, num_frames, camera_control_direction, camera_control_speed, camera_control_origin, latents, input_image):
        ...
        input_image = crop_and_resize([input_image], height, width)[0]
...
class WanVideoUnit_SpeedControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, motion_bucket_id):
...
class WanVideoUnit_UnifiedSequenceParallel(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline):
...
class WanVideoUnit_TeaCache(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, num_inference_steps, tea_cache_l1_thresh, tea_cache_model_id):
...
class WanVideoUnit_CfgMerger(PipelineUnit):
    ...
    def process(self, pipe: WanVideoReCamMasterPipeline, inputs_shared, inputs_posi, inputs_nega):
...
class WanVideoUnit_CameraEmbedder(PipelineUnit):
    ...
    def process(
        self, pipe: WanVideoReCamMasterPipeline, cam_c2w, intrinsics, height, width, num_frames, frame_interval
    ):
...
class WanVideoUnit_ReCamMasterReference(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("reference_video", "height", "width", "num_frames", "frame_interval"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoReCamMasterPipeline, reference_video, height, width, num_frames, frame_interval):
        if reference_video is None:
            return {}
        pipe.load_models_to_device(["vae"])
        reference_video = reference_video[::frame_interval][:num_frames]
        assert len(reference_video) >= num_frames, (
            f"Reference video of length={len(reference_video)} (after frame interval skipping) "
            f"is less than the given num_frames={num_frames}."
        )
        reference_video = crop_and_resize(reference_video, height, width)
        reference_latents = pipe.preprocess_video(reference_video)
        reference_latents = pipe.vae.encode(reference_latents, device=pipe.device)
        return {"reference_video_latents": reference_latents}
...
def model_fn_wan_video_recammaster(
    ...
    # ReCamMaster
    reference_video_latents: torch.Tensor = None,
    cam_emb: torch.Tensor = None,
    **kwargs,
):
    assert reference_video_latents is not None and cam_emb is not None

    if sliding_window_size is not None and sliding_window_stride is not None:
        raise NotImplementedError("ReCamMaster has not been validated with sliding window training & inference!")
        model_kwargs = dict(
            dit=dit,
        ...
        return TemporalTiler_BCTHW().run(
            model_fn_wan_video_recammaster,
    ...
    # ReCamMaster: Reference video, assuming no temporal patchify
    if reference_video_latents is not None:
        reference_video_latents, (f_reference, _, _) =\
            dit.patchify(reference_video_latents, control_camera_latents_input)
        x = torch.concat([x, reference_video_latents], dim=1)
        f += f_reference

    # ReCamMaster: Camera embedding
    if cam_emb is not None:
        # b t h w d -> b 2t h w d -> b (2t h w) d, because ReCamMaster concats source and target videos
        # TODO: What about input cameras? Should we worry about that???
        if isinstance(cam_emb, torch.Tensor):
            cam_emb = rearrange(cam_emb.repeat(1, 2, 1, 1, 1), "b f h w d -> b (f h w) d")
    ...
    if reference_video_latents is not None:
        x = x[:, :reference_video_latents.shape[1]]
        f -= f_reference
