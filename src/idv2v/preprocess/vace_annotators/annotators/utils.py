...
# import imageio.v3 as iio          # only used for quick frame-count verification
...
# my old code use moviepy to save video
# def _as_ndarray(frame: Union[np.ndarray, Image.Image]) -> np.ndarray:
#     """Convert PIL.Image to np.ndarray if necessary (RGB expected)."""
#     return np.asarray(frame) if isinstance(frame, Image.Image) else frame
# def save_one_video(
#     file_path: str,
#     videos: Sequence[Union[np.ndarray, Image.Image]],
#     fps: int = 8,
#     epsilon: float = 1e-4,            # how much to shorten the last frame
#     **write_kwargs,
# ) -> bool:
#     """
#     Save *videos* with MoviePy, guaranteeing the output has exactly
#     `len(videos)` frames (no +1 rounding glitch).

#     Parameters
#     ----------
#     file_path : str
#         Destination *.mp4* (parent dirs are created automatically).
#     videos : list[np.ndarray | PIL.Image.Image]
#         RGB frames (H,W,3) uint8.
#     fps : int, default 8
#         Frames-per-second for the output.
#     epsilon : float, default 1e-4
#         Seconds to subtract from the last frame’s nominal duration.
#         Keep it tiny so playback timing is unaffected.
#     **write_kwargs
#         Extra args for `clip.write_videofile` (e.g. codec, bitrate).
...
#         frames = [_as_ndarray(f) for f in videos]
#         n = len(frames)
#         base_dur = 1.0 / fps

#         # Make durations: all 1/fps except the last, which is slightly shorter.
#         durations = [base_dur] * n
#         durations[-1] = max(base_dur - epsilon, 0)

#         clip = ImageSequenceClip(frames, durations=durations)

#         clip.write_videofile(
#             file_path,
#             fps=fps,                  # still declare constant-fps
#             audio=False,
#             codec="libx264",
#             verbose=False,
#             # progress_bar=False,
...
#         if frame_count != n:
#             raise RuntimeError(
#                 f"Frame-count mismatch when saving video: wrote {frame_count} vs {n}."
#             )
...
#         print(f"Video save error: {e}")
#         try:
#             Path(file_path).unlink(missing_ok=True)  # remove partial file
#         except Exception:
#             pass
#         return False

# my new code uses imageio to save video
def _as_ndarray(frame: Union[np.ndarray, Image.Image]) -> np.ndarray:
    """Convert PIL.Image to np.ndarray if necessary (RGB expected)."""
    return np.asarray(frame) if isinstance(frame, Image.Image) else frame
...
def save_one_video(
    ...
    videos: Sequence[Union[np.ndarray, Image.Image]],
    fps: int = 8,
    epsilon: float = 1e-4,            # kept for API compatibility; unused with imageio
    **write_kwargs,
) -> bool:
    """
    Save *videos* with imageio (FFmpeg), ensuring the output has exactly
    `len(videos)` frames. Expects RGB uint8 frames of shape (H, W, 3).
    """
    try:
        Path(file_path).expanduser().parent.mkdir(parents=True, exist_ok=True)
        ...
        for f in videos:
            ...
            if arr.dtype != np.uint8:
                # Try to sensibly cast: support float in [0,1] or generic clip
                if np.issubdtype(arr.dtype, np.floating):
                    arr = np.clip(arr * (255.0 if arr.max() <= 1.0 else 1.0), 0, 255).astype(np.uint8)
                else:
                    arr = np.clip(arr, 0, 255).astype(np.uint8)
            if arr.ndim != 3 or arr.shape[2] != 3:
                raise ValueError(f"Each frame must be (H, W, 3) RGB uint8. Got {arr.shape} with dtype {arr.dtype}.")
            frames.append(np.ascontiguousarray(arr))

        n = len(frames)

        # Write with imageio + FFmpeg; avoid mod-16 resizing via macro_block_size=None
        # You can pass extra FFmpeg params via **write_kwargs if needed.
